...
function loadUserClips(userConfigPath) {
  ...
  const clips = {};
  for (const state of STATES) {
    if (source[state]) clips[state] = normalizeClip(source[state], state);
  }
  return clips;
}
...
function loadClipConfig({ petDir, userConfigPath }) {
  ...
  return {
    size: merged.size,
    reviewHoldMs: merged.reviewHoldMs,
    clips,
    files,
    missing,
    masks,
...
function writeUserClips(userConfigPath, userClips) {
  ...
  fs.writeFileSync(userConfigPath, `${JSON.stringify({ clips: userClips }, null, 2)}\n`);
}
