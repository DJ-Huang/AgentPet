...
const DEFAULT_VIDEO_DIRECTORIES = {
  idle: "Idle",
  waiting: "Waitting",
  working: "Work",
  completed: "Done",
...
function filesInDefaultVideoDirectory(petDir, state) {
  const directory = DEFAULT_VIDEO_DIRECTORIES[state];
  if (!directory) return null;
  const root = path.join(petDir, "Videos", directory);
  try {
  ...
  return files
    ...
    .map((file) => path.relative(petDir, file).split(path.sep).join("/"));
}
...
function loadDefaultManifest(petDir) {
  ...
  for (const state of STATES) {
    ...
    // Hana's bundled state folders are the source of truth. Keep the manifest
    // fallback for custom/legacy pets that do not use this folder layout.
    const discovered = filesInDefaultVideoDirectory(petDir, state);
    if (discovered !== null) clips[state].files = discovered;
  }
  return {
