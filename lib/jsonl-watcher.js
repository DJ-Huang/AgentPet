...
const { isPotentialMcpApprovalRecord, stateFromJsonl } = require("./state-machine");
...
function createJsonlWatcher({ machine, intervalMs = 700, approvalFallbackMs = 1200 }) {
  ...
  function scheduleApprovalFallback(sessionId, record) {
    clearPendingApproval(sessionId);
    const baselineUpdatedAt = machine.sessions?.get(sessionId)?.updatedAt || 0;
    const timer = setTimeout(() => {
      pendingApprovals.delete(sessionId);
      const currentUpdatedAt = machine.sessions?.get(sessionId)?.updatedAt || 0;
      if (currentUpdatedAt !== baselineUpdatedAt) return;
      machine.apply({
        sessionId,
        state: "waiting",
        source: "jsonl",
        event: "mcp_approval_pending",
        payload: record,
      });
    }, approvalFallbackMs);
    pendingApprovals.set(sessionId, { timer });
  }
  ...
  function applyLines(text, filePath, skipPartial) {
    ...
    for (const line of body.split(/\r?\n/)) {
      ...
      try {
        ...
        const sessionId = threadIdFromRecord(record, filePath) || fallbackId;
        if (isPotentialMcpApprovalRecord(record)) scheduleApprovalFallback(sessionId, record);
        else if (stateFromJsonl(record)) clearPendingApproval(sessionId);
        machine.applyJsonl(record, sessionId);
      } catch {
  ...
  function stop() {
    ...
    timer = null;
    for (const sessionId of pendingApprovals.keys()) clearPendingApproval(sessionId);
  }
