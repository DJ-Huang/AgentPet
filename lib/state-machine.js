...
const PRIORITY = ["working", "completed", "idle"];
...
function stateFromHookEvent(event) {
...
const PLACEHOLDER_TITLES = new Set(["agent", "ask", "edit", "plan"]);
...
function repairTitle(value) {
  ...
  try {
    const repaired = Buffer.from(str, "latin1").toString("utf8").trim();
...
class PetStateMachine {
  ...
  threadList() {
    ...
    for (const [id, session] of this.sessions) {
      ...
      const listState = running ? "working" : "completed";
    ...
    if (this.pinnedId && !seen.has(this.pinnedId) && !this.isDismissed(this.pinnedId)) {
      ...
      if (live && (RUNNING.has(live.state) || live.unreadComplete)) {
        const listState = RUNNING.has(live.state) ? "working" : "completed";
        candidates.unshift({
          id: this.pinnedId,
