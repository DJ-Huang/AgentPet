...
function isPotentialMcpApprovalRecord(record) {
  const payload = record?.payload;
  if (!payload || (payload.type !== "custom_tool_call" && payload.type !== "function_call")) return false;
  const name = String(payload.name || payload.tool_name || payload.tool || "").toLowerCase();
  if (name.startsWith("mcp__")) return true;
  const input = typeof payload.input === "string" ? payload.input : JSON.stringify(payload.input || "");
  return /(?:tools\.)?mcp__[a-z0-9_]+/i.test(input);
}
